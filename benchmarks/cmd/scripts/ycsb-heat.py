...
thread_load = 100
default_threads = [12, 36, 60, 84]
default_zipfians = [0.3, 0.5, 0.9, 0.99]
round_interval = 5
...
def run_workload_remote(node, config, mode, profile, thread, zipfian, output):
    ...
    log(f"Running workload on {node}: {profile} thread={thread} zipf={zipfian}", BLUE, config["verbose"])
...
def run_workload(config, mode, profile, thread, zipfian, output):
    if config.get("multiple_clients", False):
        ...
        log(
            ...
            f"total_thread={thread} (thread_per_node={thread_per_node}) zipf={zipfian}",
            BLUE,
            config["verbose"],
        )
    else:
        ...
        log(
            f"Running {config['wl_type']}-{config['wl_mode']} {profile} thread={thread} zipf={zipfian}",
            BLUE,
            config["verbose"],
        )
    ...
    if config.get("multiple_clients", False):
        ...
        for i, node in enumerate(client_nodes):
            ...
            remote_proc = run_workload_remote(node, config, mode, profile, thread_per_node, zipfian, node_output)
...
def load_data(config):
    # Only load for cg and oreo (skip native)
    for profile in ["cg", "oreo"]:
...
def get_metrics(config, profile, thread, zipfian):
    """
    ...
    """
    base_path = (
        f"{config['tar_dir']}/zipf_{zipfian}/{config['wl_type']}-{config['wl_mode']}-"
    ...
    if config.get("multiple_clients", False):
        ...
        print(f"\n[{profile} thread={thread} zipf={zipfian}] Node metrics:")
...
def print_summary(thread, zipfian, cg_dur, oreo_dur, cg_p99, oreo_p99, cg_ratio, oreo_ratio):
    print(f"Thread={thread}, Zipfian={zipfian}:")
    ...
    print(f"Error ratio (Abort rate):")
...
def parse_args():
    ...
    parser.add_argument("-z", "--zipfians", help="Zipfian values (comma-separated)")
...
def main():
    ...
    # Parse zipfian values
    zipfians = (
        [float(z.strip()) for z in args.zipfians.split(",")]
        if args.zipfians
        else default_zipfians
    )
    ...
    # Setup config
    wl_type = "ycsb"
    load_flag_dir = "./data/ycsb-heat"
    tar_dir = "./data/ycsb-heat"
    ...
    # Create directories for each zipfian value
    for zipfian in zipfians:
        zipf_dir = f"{config['tar_dir']}/zipf_{zipfian}"
        Path(zipf_dir).mkdir(parents=True, exist_ok=True)
    ...
    with open(results_file, "w") as f:
        f.write(
            "zipfian,thread,operation,cg,oreo,cg_p99,oreo_p99,cg_err,oreo_err\n"
        )
    ...
    log(f"Zipfian values: {zipfians}", YELLOW, config["verbose"])
    log(f"Thread counts: {threads}", YELLOW, config["verbose"])
    log(f"Total combinations: {len(zipfians) * len(threads)}", YELLOW, config["verbose"])
    ...
    # Load data if needed (only cg and oreo)
    if not load_flag_file.exists():
        load_data(config)

    # Run benchmarks - iterate through zipfian and thread combinations
    total_runs = len(zipfians) * len(threads) * 2  # 2 profiles: cg, oreo
    current_run = 0
    
    for zipfian in zipfians:
        log(f"\n{'='*60}", PURPLE, True)
        log(f"Testing Zipfian = {zipfian}", PURPLE, True)
        log(f"{'='*60}\n", PURPLE, True)
        
        for thread in threads:
            current_run += 2  # cg and oreo
            log(f"Progress: {current_run}/{total_runs} runs completed", CYAN, True)
            
            # Only test cg and oreo (skip native)
            for profile in ["cg", "oreo"]:
                output = (
                    f"{config['tar_dir']}/zipf_{zipfian}/{wl_type}-{args.workload}-"
                    f"{args.db}-{profile}-{thread}.txt"
                )
                run_workload(config, "run", profile, thread, zipfian, output)

            cg_dur, cg_p99, cg_ratio = get_metrics(config, "cg", thread, zipfian)
            oreo_dur, oreo_p99, oreo_ratio = get_metrics(config, "oreo", thread, zipfian)

            # Append results
            with open(results_file, "a") as f:
                f.write(
                    f"{zipfian},{thread},{operation},{cg_dur:.4f},{oreo_dur:.4f},"
                    f"{cg_p99},{oreo_p99},"
                    f"{cg_ratio:.4f},{oreo_ratio:.4f}\n"
                )
    ...
    log(f"\n{'='*60}", GREEN, True)
    log("All benchmarks completed!", GREEN, True)
    log(f"Results saved to: {results_file}", GREEN, True)
    log(f"{'='*60}\n", GREEN, True)
